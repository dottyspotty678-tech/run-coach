...
| Watch sync line | `components/watch-sync.tsx` (`WatchSyncLine`), status from `components/watch-status.ts`, rendered in both Plan week cards | Watch sync (WATCH-SYNC §4, DESIGN §8f): one line under the season position — "On watch · 3 runs, 2 gym · synced 2 min ago" (bracken tick), "Not on watch yet" (tertiary), or "Watch: {last_error} · last good push …" (danger) — with a compact round Send/Resend button calling `pushWeekToWatch(weekStart)` (spinner, refresh on return, surfaced failure). The runs/gym breakdown counts the plan's session types. The whole line is hidden when `isIntervalsConfigured()` is false. |
| intervals.icu card | `components/watch-sync.tsx` (`WatchCard`), rendered in Settings → Connections | Matches the Strava/Microsoft cards: "Connected" (key present) or "Not configured" with the one line "Add INTERVALS_ICU_API_KEY in Vercel"; this week's last push time + workout count from `getWatchSync(boundaryWeekStart())`; "Send this week" (secondary button, hidden when not configured); last-push failure slot (prop named `issue`, omitted when clean — cos-1); one-line note that Coros picks it up via intervals.icu's planned-workout upload. |
