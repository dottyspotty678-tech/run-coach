...
- Include exactly two strength sessions this week (session_type "strength"): one on a weekday and one on a weekend day, defaulting to Tuesday and Sunday unless the calendar makes those impossible — then the nearest sensible weekday/weekend day. With Sunday as a gym day, the long run defaults to Saturday; do not stack the long run and a strength session on the same day.
